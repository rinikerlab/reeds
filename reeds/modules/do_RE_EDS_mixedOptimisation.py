...
Description:
    This script executes multiple simulations. The scripts runs mutliple iterations of parameter optimization runs.
    In each optimization iteration, both the energy offsets are updated with the energy offset rebalancing scheme, 
    and the s-distribution with with the GRTO or LRTO algorithm.
    
    Default settings are to run a 1ns simulation with exchanges every 50 steps, for which we discard the first 100ps as 
    equilibration.
...
def do(out_root_dir: str, in_simSystem: fM.System, in_template_imd: str = None,
       ...
       optimized_states_dir: str = os.path.abspath("a_optimizedStates/analysis/next"),
       ...
       equil_runs: int = 0, prod_runs: int  = 1, steps_between_trials: int = 50, 
    ...
    if (verbose): print(spacer + "\n\tSTART mixed optimization process.")
    #################
    # Prepare general stuff
    #################
    optimization_name = "mixedOpt"
    ...
    job_id = do_optimization(out_root_dir=out_root_dir, in_simSystem=in_simSystem, optimization_name=optimization_name, in_template_imd=in_template_imd,
                            ...
                            sOpt_add_replicas= sOpt_add_replicas, sOpt_adding_new_sReplicas_Scheme= adding_Scheme_new_Replicas,
