...
class EDSSimulation(app.Simulation):
  ...
  def create_system(self, platform = None, properties = None):
    ...
    sys.stdout.flush()
    self.integrator = mm.LangevinMiddleIntegrator(self.temperature, 1/u.picoseconds, 0.002*u.picoseconds)
  ...
  def create_reaction_field(self):
    ...
    # CustomCVForce
    energy = "-1/(beta*s)*expsum;"
    for i in reversed(range(2, self.num_endstates)):
      energy += f"expsum = max(part{i},expsum) + log(1+exp(min(part{i}, expsum) - max(part{i},expsum)));"
      energy += f"part{i} = -beta * s * (v{i} - eoff{i});"

    energy += "expsum = max(part0, part1) + log(1+exp(min(part0,part1) - max(part0, part1)));"
    energy += "part0 = -beta * s * (v0 - eoff0);"
    energy += "part1 = -beta * s * (v1 - eoff1);"

    for i in range(self.num_endstates):
      energy += f"v{i} = lj_rf_endstate_{i} + lj_rf_endstate_{i}_one_four + rf_endstate_{i}_excluded + rf_endstate_{i}_self_interaction;"

    energy += "beta = {:f};".format(self.beta)

    cvforce = mm.CustomCVForce(energy)
    cvforce.addGlobalParameter("s", self.s_value)
    for i, eoff in enumerate(self.energy_offsets):
      cvforce.addGlobalParameter(f"eoff{i}", eoff)
      
    # custom reaction field
    for i, active_particles in enumerate(active_particles_):
        if(active_particles != environment_particles):
          ...
          cvforce.addCollectiveVariable("lj_rf_endstate_" + str(i), a)
          cvforce.addCollectiveVariable("lj_rf_endstate_" + str(i) + "_one_four", b)          
          cvforce.addCollectiveVariable("rf_endstate_" + str(i) + "_excluded", c)   
          cvforce.addCollectiveVariable("rf_endstate_" + str(i) + "_self_interaction", d)
        else:
          a, b, c, d = (self.custom_reaction_field(i, default_nonbonded_force, active_particles, environment_particles))
        ...
        self.system.addForce(deepcopy(a))

        if (b.getNumBonds()):
            self.system.addForce(deepcopy(b))
            
        if (c.getNumBonds()): 
            self.system.addForce(deepcopy(c))
            
        if (d.getNumBonds()):
            self.system.addForce(deepcopy(d))
    
    cvforce.setForceGroup(1)
    self.system.addForce(cvforce)
    self.context.reinitialize() 
    
    self.integrator.setIntegrationForceGroups({0,1})
  ...
  def get_VR(self):
    return self.context.getState(getEnergy=True, groups = 1<<1).getPotentialEnergy().value_in_unit(u.kilojoules_per_mole)

  def get_Vi(self):
    return [self.context.getState(getEnergy=True, groups=1<<i+2).getPotentialEnergy().value_in_unit(u.kilojoules_per_mole) for i in range(self.num_endstates)]    
...
class REEDS:
  ...
  def logsumexp_(self, s, Vi):
    ...
    """
    partA = -self.EDS_simulation.beta * s * (Vi[0] - self.EDS_simulation.energy_offsets[0])       
    partB = -self.EDS_simulation.beta * s * (Vi[1] - self.EDS_simulation.energy_offsets[1])
    sum_prefactors = max(partA, partB) + np.log(1+np.exp(min(partA, partB) - max(partA, partB)))
    
    prefactors = np.array([0.] * self.EDS_simulation.num_endstates)
    prefactors[0] = partA
    prefactors[1] = partB
    
    for i in range(2, self.EDS_simulation.num_endstates):
      part = -self.EDS_simulation.beta * s * (Vi[i] - self.EDS_simulation.energy_offsets[i])
  ...
  def perform_replica_exchanges(self):
    ...
    if self.rank == 0:
      ...
      for i in range(self.begin, self.num_replicas-1,2):
        ...
        V_orig_p1 = -1/(self.EDS_simulation.beta * self.s_values[p1]) * self.logsumexp_(self.s_values[p1], self.Vi_all[i])
        V_exch_p1 = -1/(self.EDS_simulation.beta * self.s_values[p2]) * self.logsumexp_(self.s_values[p2], self.Vi_all[i])

        V_orig_p2 = -1/(self.EDS_simulation.beta * self.s_values[p2]) * self.logsumexp_(self.s_values[p2], self.Vi_all[i+1])
        V_exch_p2 = -1/(self.EDS_simulation.beta * self.s_values[p1]) * self.logsumexp_(self.s_values[p1], self.Vi_all[i+1])
        ...
        if(p1 != 0):
          ...
          self.EDS_simulation.context.setParameter("s", self.EDS_simulation.s_value)
          self.EDS_simulation.energy_offsets = self.energy_offset_matrix[p1]
          for j in range(self.EDS_simulation.num_endstates):
            self.EDS_simulation.context.setParameter(f"eoff{j}", self.EDS_simulation.energy_offsets[j])
    ...
    # replicas != 0 receive their current s values
      ...
      self.EDS_simulation.context.setParameter("s", self.EDS_simulation.s_value)
      for i in range(self.EDS_simulation.num_endstates):
        self.EDS_simulation.context.setParameter(f"eoff{i}", self.EDS_simulation.energy_offsets[i])
