...
class EDSInputFiles:                                                                                  
  ...
  """
  def __init__(self, parameter_file, coordinate_file, ptp_file = None, state_file = None):
    self.parameter_file = parameter_file
    self.coordinate_file = coordinate_file
    self.ptp_file = ptp_file
...
class REEDSInputFiles:
  ...
  """
  def __init__(self, parameter_file, coordinate_files, ptp_file = None, state_files = None):
    for i in range(MPI.COMM_WORLD.Get_size()):
      if MPI.COMM_WORLD.Get_rank() == i:
        if isinstance(coordinate_files, list):
          if isinstance(state_files, list):
            self.eds_input_files = EDSInputFiles(parameter_file, coordinate_files[i], ptp_file, state_files[i])
          else:
            self.eds_input_files = EDSInputFiles(parameter_file, coordinate_files[i], ptp_file, state_files)
        elif isinstance(state_files, list):
          self.eds_input_files = EDSInputFiles(parameter_file, coordinate_files, ptp_file, state_files[i])
        else:
          self.eds_input_files = EDSInputFiles(parameter_file, coordinate_files, ptp_file, state_files)
...
class EDSSimulation(app.Simulation):
  ...
  def create_reaction_field(self):
    ...
    if self.eds_input_files.ptp_file == None:
      ...
      perturbed_particles_ = [self.context.getMolecules()[i] for i in range(self.num_endstates)]
      perturbed_particles_.append(environment_particles)            
      
      # custom forces

      for i, perturbed_particles in enumerate(perturbed_particles_):
          if(perturbed_particles != environment_particles):
            a, b, c, d = (self.custom_reaction_field(i, default_nonbonded_force, perturbed_particles, environment_particles))
      ...
      spec = importlib.util.spec_from_file_location("perturbations", self.eds_input_files.ptp_file)
      perturbed_particles = importlib.util.module_from_spec(spec)
      sys.modules["perturbations"] = perturbed_particles
      spec.loader.exec_module(perturbed_particles)
      self.topology_type = perturbed_particles.topology_type
      self.perturbed_particles = perturbed_particles.perturbed_particles
      environment_particles = []
      
      for i in range(self.system.getNumParticles()):
        if i not in self.perturbed_particles:
          environment_particles.append(i)
      
      for i in range(self.num_endstates):
        a, b, c, d = (self.custom_reaction_field_ptp(i, default_nonbonded_force, self.perturbed_particles, environment_particles))
        a.setName(f"lj_rf_endstate_{i+1}")
        b.setName(f"lj_rf_endstate_{i+1}_one_four")
        c.setName(f"rf_endstate_{i+1}_excluded")
        d.setName(f"rf_endstate_{i+1}_self_interaction")
      ...
      if(len(environment_particles)):      
        a, b, c, d = self.custom_reaction_field_ptp(i+1, default_nonbonded_force, environment_particles, environment_particles)
        self.system.addForce(a)
  ...
  def custom_reaction_field(self, end_state, original_nonbonded_force, perturbed_particles, environment_particles):
    """
    ...
    force_lj_crf.addInteractionGroup(perturbed_particles, environment_particles)
    if(perturbed_particles != environment_particles):
      force_lj_crf.addInteractionGroup(perturbed_particles, perturbed_particles)
    ...
    for index in range(original_nonbonded_force.getNumExceptions()):
      ...
      if j in perturbed_particles and k in perturbed_particles and (chargeprod._value != 0 or epsilon._value != 0 ):
    ...
    for index in range(original_nonbonded_force.getNumExceptions()):
      j, k, chargeprod, sigma, epsilon = original_nonbonded_force.getExceptionParameters(index)
      if j in perturbed_particles and k in perturbed_particles and (chargeprod._value == 0):
        ch1, _, _ = original_nonbonded_force.getParticleParameters(j)
        ch2, _, _ = original_nonbonded_force.getParticleParameters(k)
  ...
  def custom_reaction_field_ptp(self, end_state, original_nonbonded_force, perturbed_particles, environment_particles):
    """
    defines a reaction field with a shifting function (see A. Kubincova et al, Phys. Chem. Chem. Phys. 2020, 22)
    ...
    # nonbonded pairlist interactions (lennard jones and coulomb reaction field)
    #
    lj_crf  = scal + "*(4*epsilon*(sigma_over_r12 - sigma_over_r6) + ONE_4PI_EPS0*chargeprod*(one_over_r + krf*r2 + arfm*r4 + arfn*r6 - crf));"
    lj_crf += "krf = {:f};".format(krf.value_in_unit(u.nanometer**-3))
    lj_crf += "crf = {:f};".format(crf.value_in_unit(u.nanometer**-1))
    lj_crf += "one_over_r = select(chargeprod, 1/r, 0);"
    lj_crf += "r6 = r2*r4;"
    lj_crf += "r4 = r2*r2;"
    lj_crf += "r2 = r*r;"
    lj_crf += "arfm = {:f};".format(arfm.value_in_unit(u.nanometer**-5))
    ...
    lj_crf += "sigma_over_r6 = sigma_over_r3 * sigma_over_r3;"
    lj_crf += "sigma_over_r3 = sigma_over_r * sigma_over_r * sigma_over_r;"
    lj_crf += "sigma_over_r = select(epsilon, sigma/r, 0);"
    lj_crf += "epsilon = sqrt(epsilon1*epsilon2);"
    lj_crf += "sigma = 0.5*(sigma1+sigma2);"
    lj_crf += "chargeprod = charge1*charge2;"
    lj_crf += "ONE_4PI_EPS0 = {:f};".format(ONE_4PI_EPS0)
    ...
    # overwrite perturbed parameters for current end-state
    if perturbed_particles != environment_particles:
      for perturbed_particle in perturbed_particles:
        if self.topology_type == "amber":
          parameters = perturbed_particles[perturbed_particle][end_state]
          charge = parameters[0] / 18.2223
          if parameters[1] != 0 and parameters[2] != 0:
            sigma = ((parameters[1] / parameters[2])**(1/6)) * 0.1
            epsilon = (parameters[2]**2 / ( 4 * parameters[1] )) * 4.184
          else:
            sigma = 0.0
            epsilon = 0.0

          print(perturbed_particle, charge, sigma, epsilon)
          force_lj_crf.setParticleParameters(perturbed_particle, [charge, sigma, epsilon])
        else:
          print(f"Error: topology type {self.topology_type} not implemented")
          sys.stdout.flush()
          self.comm.Abort()
    ...
    # set interaction groups -> end-state with itself and end-state with environment (or only environment with environment)
    pp = [p for p in perturbed_particles]
    force_lj_crf.addInteractionGroup(pp, environment_particles)
    if(perturbed_particles != environment_particles):
      force_lj_crf.addInteractionGroup(pp, pp)
    ...
    if perturbed_particles != environment_particles:
      for index in range(original_nonbonded_force.getNumExceptions()):
        ...
        if j in perturbed_particles and k in perturbed_particles and (chargeprod._value != 0 or epsilon._value != 0 ):
          if self.topology_type == "amber":
            parameters1 = perturbed_particles[j][end_state]
            charge1 = parameters1[0] / 18.2223
            if parameters1[1] != 0 and parameters1[2] != 0:
              sigma1 = (parameters1[1] / parameters1[2])**(1/6) * 0.1
              epsilon1 = parameters1[2]**2 / ( 4 * parameters1[1] ) * 4.184
            else:
              sigma1 = 0.0
              epsilon1 = 0.0

            parameters2 = perturbed_particles[k][end_state]
            charge2 = parameters2[0] / 18.2223
            if parameters2[1] != 0 and parameters2[2] != 0:
              sigma2 = (parameters2[1] / parameters2[2])**(1/6) * 0.1
              epsilon2 = parameters2[2]**2 / ( 4 * parameters2[1] ) * 4.184
            else:
              sigma2 = 0.0
              epsilon2 = 0.0

            force_lj_crf_one_four.addBond(j, k, [charge1 * charge2 / 1.2, 0.5 * (sigma1+sigma2), np.sqrt(epsilon1*epsilon2) * 0.5 , charge1*charge2])
      ...
      for index in range(original_nonbonded_force.getNumExceptions()):
        ...
        if j in environment_particles and k in environment_particles and (chargeprod._value != 0 or epsilon._value != 0 ):
          ch1, _, _ = original_nonbonded_force.getParticleParameters(j)
          ch2, _, _ = original_nonbonded_force.getParticleParameters(k)
          force_lj_crf_one_four.addBond(j, k, [chargeprod, sigma, epsilon, ch1*ch2])
    ...
    if perturbed_particles != environment_particles:
      for index in range(original_nonbonded_force.getNumExceptions()):
        ...
        if j in perturbed_particles and k in perturbed_particles and (chargeprod._value == 0):
          if self.topology_type == "amber":
            ...
            force_crf_excluded.addBond(j, k, [charge1*charge2])
      ...
      for index in range(original_nonbonded_force.getNumExceptions()):
        j, k, chargeprod, sigma, epsilon = original_nonbonded_force.getExceptionParameters(index)
        if j in environment_particles and k in environment_particles and (chargeprod._value == 0):
          ch1, _, _ = original_nonbonded_force.getParticleParameters(j)
          ch2, _, _ = original_nonbonded_force.getParticleParameters(k)
          force_crf_excluded.addBond(j, k, [ch1*ch2])
    ...
    if perturbed_particles != environment_particles:
      for perturbed_particle in perturbed_particles:
        parameters = perturbed_particles[perturbed_particle][end_state]
        charge = parameters[0] / 18.2223
        force_crf_self_term.addBond(perturbed_particle, perturbed_particle, [charge * charge])
    else:
      for i in environment_particles:
        ch1, _, _ = original_nonbonded_force.getParticleParameters(i)
        force_crf_self_term.addBond(i, i, [ch1*ch1])
