...
class EDSSimulation(app.Simulation):
  ...
  def get_VR(self):
    return -1/(self.beta * self.s_value) * self.integrator.getGlobalVariableByName("expsum")
...
class EDSIntegrator(mm.CustomIntegrator):
  ...
  and the calculation of the exponential terms of https://doi.org/10.1016/S0010-4655(03)00245-5
  """
  def __init__(self, s, time_step, beta, energy_offsets):

    mm.CustomIntegrator.__init__(self, time_step)

    friction = 1 / u.picoseconds
    num_endstates = len(energy_offsets)

    self.addGlobalVariable("a", np.exp(-friction*time_step))
    self.addGlobalVariable("b", np.sqrt(1-np.exp(-2*friction*time_step)))
    self.addGlobalVariable("kT", 1/beta)
    self.addGlobalVariable("beta", beta)
    for i in range(num_endstates):
      self.addGlobalVariable("eoff" + str(i), energy_offsets[i])
    self.addGlobalVariable("s", s)
    self.addPerDofVariable("x1", 0)

    self.addUpdateContextState()

    self.addGlobalVariable("part0", 1.0)
    self.addGlobalVariable("part1", 1.0)

    self.addComputeGlobal("part0", "-beta*s*(energy1-eoff0)")
    self.addComputeGlobal("part1", "-beta*s*(energy2-eoff1)")
